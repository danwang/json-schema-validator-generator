    ...
    if (typeof schema.type === 'string') {
      switch (schema.type) {
        ...
        case 'string':
          return Ast.String;
        case 'object':
          return makeObjectAst(schema, refResolver);
        case 'array':
          return makeArrayAst(schema, refResolver);
        default:
          return Ast.Mixed;
      }
    } else {
...
const makeObjectAst = (
  ...
  if (schema.properties) {
    ...
    return Ast.Record(
      _.mapValues(schema.properties, (subSchema, key) => {
...
export default (
  schema: JsonSchema,
  refResolver: RefResolver = defaultResolver
) => {

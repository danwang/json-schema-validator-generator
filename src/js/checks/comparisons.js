...
const comparisons = (
  ...
  const keysLength = Ast.PropertyAccess(
    Ast.Call1('Object.keys', symbol),
    'length'
  );

  const comparison = (sym: JsAst, comparator: Binop, field: Field): JsAst => {
  ...
  return Ast.Body(
    M.TypeCheck(
      ...
      Ast.Body(
        comparison(
          symbol,
          schema.exclusiveMinimum ? Ast.Binop.Lte : Ast.Binop.Lt,
          'minimum'
        ),
        comparison(
          symbol,
          schema.exclusiveMaximum ? Ast.Binop.Gte : Ast.Binop.Gt,
          'maximum'
        )
      )
    ),
